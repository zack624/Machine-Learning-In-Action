...
# 数据点绘制
def plot(dataMat,labelMat,svMat,ws=None,b=0):
    ...
    ax.scatter(svXcord,svYcord,s=30,c='blue',marker='^')
    if (b != 0): # 绘制求得的分类器图像
        x = arange(2.0,7.0,1.0)
        y = (-b-ws[0]*x)/ws[1]
        ax.plot(x,y)
...
def testRbf(k1=1.3):
    ...
    for i in range(m):
        ...
        if sign(predict) != sign(labelArr[i]): errorCount += 1 # sign函数
